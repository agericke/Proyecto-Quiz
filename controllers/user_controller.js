...
exports.index = function(req, res, next) {
    models.User.findAll({order: ['username']})
        .then(function(users) {
            res.render('users/index', { users: users, title: "Usuarios" });
        })
        .catch(function(error) { next(error); });
};
...
exports.show = function(req, res, next) {
    res.render('users/show', {user: req.user, title: "Usuario:"+req.user.username});
};
...
exports.new = function(req, res, next) {
    ...
    res.render('users/new', { user: user, title:"Nuevo usuario" });
};
    ...
    models.User.find({where: {username: req.body.user.username}})
        .then(function(existing_user) {
            if (existing_user) {
                ...
                res.render('users/new', { user: user, title:"NuevoUsuario" });
            } else {
                // Guardar en la BBDD
                return user.save({fields: ["username", "password", "salt"]})
                    ...
                    .catch(Sequelize.ValidationError, function(error) {
                        ...
                        res.render('users/new', { user: user, title:"NuevoUsuario2" });
                    });
            }
        })
...
exports.edit = function(req, res, next) {
    res.render('users/edit', { user: req.user, title:"Editar usuario" });  // req.user: instancia de user cargada con autoload
};            
...
exports.update = function(req, res, next) {
    ...
    req.user.save({fields: ["username", "password", "salt"]})
        ...
        .catch(Sequelize.ValidationError, function(error) {
            ...
            res.render('users/edit', {user: req.user, title:"Volver a Editar"});
        })
        .catch(function(error) {
            next(error);
        });
